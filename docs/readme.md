...
`guise` gives you a themed palette, sizing tokens, ~40 ready-made components,
a Flutter-style flexbox layer, terse layout macros, and a lightweight
React-style state layer — all on top of gpui's retained-mode renderer.
...
- [Inputs](inputs.md) — `TextInput`, `Checkbox`, `Switch`, `Radio`, `Select`, `SegmentedControl`, `Chip`
- [Typography](typography.md) — `Text`, `Title`, `Anchor`, `Code`, `Kbd`
- [Layout](layout.md) — `Stack`, `Group`, `Center`, `Paper`, `Card`, `Divider`
- [Feedback](feedback.md) — `Alert`, `Loader`, `Progress`, `Notification`, `Skeleton`
- [Data display](data.md) — `Avatar`, `AvatarGroup`, `Badge`, `Indicator`, `List`, `Table`, `Tabs`, `Accordion`
- [Overlays](overlays.md) — `Modal`, `Menu`, `Tooltip`
...
- [Reactive state](reactive.md) — `Signal`, `provide`/`use_context`, `use_state`/`watch`
